        ...
        while(loop_closer_running.load())
        {
            ...
            if(database.size()>10)
            {
                ...
                for(std::size_t s = 0; s < similarity.size(); s++)
                {
                    ...
                    if(similarity.at(s).Score > 0.03 && similarity.at(s).Score < 0.95) 
                    {
                        ...
                        if(abs(current_frame->keyframe_id - matched_kf->keyframe_id) > 20)
                        {
                            double x1 = current_frame->getPose().inverse().matrix3x4().coeff(0,3); 
                            double y1 = current_frame->getPose().inverse().matrix3x4().coeff(2,3); 

                            double x2 = matched_kf->getPose().inverse().matrix3x4().coeff(0,3); 
                            double y2 = matched_kf->getPose().inverse().matrix3x4().coeff(2,3); 
            ...
            std::cout  << "Loop time for loop closer: " << elapsedT.count() << " ms. \n";
        }
    }
