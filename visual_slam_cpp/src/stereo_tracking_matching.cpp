...
namespace myslam {
    ...
    int StereoTracking_Match::TrackLastFrame() 
    {
        ...
        */
        ...
        std::vector<cv::DMatch> matches_current_lr, matches_prev_current, good_matches_prev_curr; 
        unsigned int found_features = 0; 
        
        //get descriptors from previous frame 
        for(std::size_t i = 0; i < last_frame->features_left_.size(); i++)
        {
            descriptors_last_left.push_back(last_frame->features_left_[i]->descriptor); 
        }
        ...
        //! match features from prev left img with features in left current img
        matcher->match(descriptors_current_left, descriptors_last_left, matches_prev_current); 

        double min_dist=10000, max_dist=0;
        for ( std::size_t i = 0; i < matches_prev_current.size(); i++ )
        {
            double dist = matches_prev_current[i].distance;
        ...
        {
            if ( matches_prev_current[i].distance <= std::max( 2*min_dist, 30.0 ) )
            {
                good_matches_prev_curr.push_back(matches_prev_current[i]);
                good_descriptors_current.push_back(descriptors_current_left.row(matches_prev_current[i].queryIdx)); 
            }
        }

        // //! match good matches from prev to current img with features in current right im
        matcher->match(good_descriptors_current, descriptors_current_right, matches_current_lr); 
        ...
        {
            if ( matches_current_lr[i].distance <= std::max( 2*min_dist, 30.0 ) )
            {
                found_features++; 
                current_frame->features_left_.emplace_back(new Feature(current_frame, kps_left.at(matches_current_lr[i].queryIdx), 
                                                                        good_descriptors_current.row(matches_current_lr[i].queryIdx))); 
                                                                                             
                current_frame->features_right_.emplace_back(new Feature(current_frame,kps_right.at(matches_current_lr[i].trainIdx), 
                                                                        descriptors_current_right.row(matches_current_lr[i].trainIdx), false)); 
            }
        }
    ...
    int StereoTracking_Match::extractStereoFeatures()
    {
        ...
        */
        ...
        for (std::size_t i = 0; i < matched_points.size(); i++ )
        {
            ...
            {
                found_features++; 
                current_frame->features_left_.emplace_back(new Feature(current_frame, kps_left.at(matched_points[i].queryIdx), 
                                                                        descriptors_left.row(matched_points[i].queryIdx))); 
                                                                                             
                current_frame->features_right_.emplace_back(new Feature(current_frame,kps_right.at(matched_points[i].trainIdx), 
                                                                        descriptors_right.row(matched_points[i].trainIdx), false)); 
